...
class UI(QMainWindow):
        ...
        if CONNECTION:
            ...
            try:
                ...
                if packet != b'':
                    ...
                    try:
                        print(packet)
                        for i in range(0, len(packet)): #PROMBLEM HERE TODO
                            if i != chr(92):
    ...
    def connect(self):
        ...
        port = None #Port must be initialized
        print(port)
        available_ports = list(serial.tools.list_ports.comports())
        print([comport.device for comport in serial.tools.list_ports.comports()])
        for port in available_ports:
            print(port.device)
            if "COM14" in str(port): #Name of hardware in string, must be renamed for each device
